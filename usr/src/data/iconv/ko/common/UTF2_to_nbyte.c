...
#pragma	ident	"$Id: UTF2_to_nbyte.c,v 1.12 1998/10/12 16:18:30 hemant Exp $ SMI"
...
void echo_vowel(char* c,  int* i)
{
	...
	else if (c[*i] == '{')  /* _| */
	{
		...
		(*i)++;
}  /* end of void echo_vowel(char*, int*). */


/****  E C H O _ C O N S O N A N T  ****/

void echo_consonant(char* c,  int* i)
{
	if (c[*i] == 'C')  /* gs */
	{
		c[(*i)++] = 'A';	/* g */
	...
	else if (c[*i] == 'T')  /* bs */
	{
		...
		(*i)++;
}  /* end of void echo_consonant(char*, int*). */
