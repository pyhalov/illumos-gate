...
include $(SRC)/data/iconv/utf-8/Makefile.iconv
...
install: all $(ROOT_DIR) $(BTS_DIR) $(ROOT_DIR)/alias
	...
	for f in $(DYNOBJS) ; do \
		...
		$(RM) $(ROOT_DIR)/$$TMP ; \
		$(CP) $$f $(ROOT_DIR)/$$TMP ; \
		$(CHMOD) 555 $(ROOT_DIR)/$$TMP ; \
	done
	-@echo "done."


$(ROOT_DIR) :
	/usr/bin/test -d $@ || $(INS.dir) $@
	/usr/bin/test -d $@/$(MACH64) || $(INS.dir) $@/$(MACH64)
...
$(DYNOBJS)		:= CFLAGS64 += $(XREGSFLAG64) -fpic -D_REENTRANT \
					-I$(COMMON) -I$(COMMON)/tbls
